...
import { HelpCircle, Mic, BrainCircuit, Sparkles, MousePointerClick, Zap, Activity, MessageSquare, Hand, ListChecks, Search, Globe, Mail } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 pb-32 overflow-y-auto animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-4 mb-10 pb-6 border-b border-gray-200">
        <div className="bg-blue-100 p-3 rounded-full"><HelpCircle className="w-8 h-8 text-doc-accent" /></div>
        <div>
          <h1 className="text-3xl font-serif font-bold text-gray-800">{h.title}</h1>
          ...
          <div className="grid sm:grid-cols-2 gap-4 relative z-10">
            {h.goldenPath.steps.map((item, idx) => (
              <div key={idx} className="bg-white/5 p-5 rounded-2xl border border-white/10 backdrop-blur-sm hover:bg-white/10 transition-colors">
                ...
                <div className="text-[11px] text-slate-400 leading-relaxed font-medium">{item.text}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="text-center p-8 bg-slate-900 text-white rounded-[40px] border border-blue-500/20">
        <h2 className="text-2xl font-serif font-bold mb-3">{h.moreHelp}</h2>
        <p className="text-slate-400 mb-6 text-sm">{h.moreHelpDesc}</p>
